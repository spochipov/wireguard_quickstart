...
echo "./wg-debug-internet.sh"
./wg-debug-internet.sh 2>&1 | tee "$OUT_DIR/wg-debug-internet.txt"

echo
echo "=== 2) Снимки правил (iptables, ip6tables) ==="
echo "iptables-save"
iptables-save 2>&1 | tee "$OUT_DIR/iptables-save.txt"

echo
echo "ip6tables-save"
ip6tables-save 2>&1 | tee "$OUT_DIR/ip6tables-save.txt"

echo
echo "=== 3) Если используется nftables (необязательно) ==="
echo "nft list ruleset || true"
nft list ruleset 2>&1 | tee "$OUT_DIR/nft-ruleset.txt" || true

echo
echo "=== 4) Подробно: таблицы iptables ==="
echo "iptables -L INPUT -n --line-numbers"
iptables -L INPUT -n --line-numbers 2>&1 | tee "$OUT_DIR/iptables-INPUT.txt"

echo
echo "iptables -L FORWARD -n --line-numbers"
iptables -L FORWARD -n --line-numbers 2>&1 | tee "$OUT_DIR/iptables-FORWARD.txt"

echo
echo "iptables -t nat -L POSTROUTING -n --line-numbers"
iptables -t nat -L POSTROUTING -n --line-numbers 2>&1 | tee "$OUT_DIR/iptables-nat-POSTROUTING.txt"

echo
echo "=== 5) Проверка маршрутизации / исходящего трафика с интерфейса wg0 ==="
echo "ip route get 8.8.8.8 from 10.174.38.1"
ip route get 8.8.8.8 from 10.174.38.1 2>&1 | tee "$OUT_DIR/ip-route-get.txt"
